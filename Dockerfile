...
RUN mkdir /tmp/mws && \
	cd /tmp/mws && \
	wget -q https://github.com/shabiel/M-Web-Server/archive/refs/tags/1.1.2.zip -O mws.zip && \
...
# docker run -d --name=ydbguidev -p 8089:8089 -v $PWD/wwwroot:/YDBGUI/wwwroot:rw -v $PWD/routines:/YDBGUI/routines:rw ydbgui
